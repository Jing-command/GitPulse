...
async function takeScreenshot(page, name) {
  const filePath = path.join(SCREENSHOT_DIR, `settings-${name}.png`);
...
(async () => {
  ...
  try {
    ...
    await takeScreenshot(page, '01-settings-page');

    const currentUrl = page.url();
    if (currentUrl.includes('/settings')) {
      results.passed.push('成功进入设置页面');
      console.log('   ✅ 成功进入设置页面');
    } else {
      results.failed.push('未能进入设置页面');
      console.log('   ❌ 未能进入设置页面');
    }

    console.log('📋 测试 2: 验证页面标题');
    const pageTitle = page.locator('h1:has-text("设置")');
    if (await pageTitle.isVisible({ timeout: 3000 }).catch(() => false)) {
      results.passed.push('设置页面标题显示正常');
      console.log('   ✅ 设置页面标题显示正常');
    } else {
      results.warnings.push('未检测到设置页面标题');
      console.log('   ⚠️ 未检测到设置页面标题');
    }
